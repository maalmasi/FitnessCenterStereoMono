...
class ScheduleViewStore {
    ...
    @observable sortBy = "frequency";
    @observable sortAsc = true;
    @observable searchQuery = "";

    @action.bound onCreate() {
        this.routerStore.goTo("schedulecreate");
    };

    @action.bound onUpdate(id) {
        this.routerStore.goTo("scheduleedit", { id: id });
    }
    ...
    resultsToArray = () => {
        ...
        for (let i = 0; i < this.resultItems.items.length; i++) {
            this.displayItems.push({
                ...
                frequency: this.resultItems.items[i].frequency,
                planid: this.resultItems.items[i].planId
            });
        }
    }
